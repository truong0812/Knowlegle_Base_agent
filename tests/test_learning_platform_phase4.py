...
def test_learning_api_accepts_injected_path_planner_factory(tmp_path: Path):
            ...
            return type("PlanningResult", (), {
                "paths": [
                    LearningPathDetail(
                        id="custom-path",
                        title="Custom path",
                        description="Generated by an injected planner.",
                        lessons=[
                            LearningLesson(
                                id="lesson-1",
                                title="Custom lesson",
                                summary="Learn through the injected planner.",
                                explanation="This verifies LearningApi only depends on the planner interface.",
                            )
                        ],
                    )
                ],
    ...
    def factory(nodes, edges, hotpath):
        captured["node_count"] = len(nodes)
        captured["edge_count"] = len(edges)
        captured["hotpath_count"] = len(hotpath)
        return FakePlanner()

    api = LearningApi(kb, path_planner_factory=factory)

    paths = api.paths()

    assert paths[0].id == "custom-path"
    assert captured == {"node_count": 6, "edge_count": 4, "hotpath_count": 3}


def test_learning_api_returns_empty_paths_when_graph_load_fails(tmp_path: Path):
    kb = tmp_path / "repo" / ".kb"
    kb.mkdir(parents=True)
    (kb / "manifest.json").write_text(json.dumps({"source_repo": str(kb.parent)}), encoding="utf-8")

    class MissingGraphStorage:
        def load(self):
            raise FileNotFoundError("missing graph")

        def load_hotpath(self):
            raise AssertionError("hotpath should not be loaded without graph")

        def load_features(self):
            return []

    api = LearningApi(kb, storage=MissingGraphStorage())
